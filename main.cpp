...
static bool bin_search(const AlignedIntArray& hayStack, const int needle, uint64_t& left, uint64_t count) {
	// Binary search
	while (count != 0) {
		...
		if (hayStack[mid] < needle) {
			left = mid + 1;
			count -= half + 1;
			// New left value for range
		}
		else {
			count = half;
			// New right val for range
		}
	}
	// On the last step of while loop - half == 0
...
static void betterSearch(const AlignedIntArray& hayStack, const AlignedIntArray& needles, AlignedIntArray& indices, StackAllocator& allocator) {
	const int haystack_max = hayStack[(int)hayStack.getCount() - 1];
	const int haystack_min = hayStack[0];
	const uint64_t hayStackSize = hayStack.getCount();
	const uint64_t needlesSize = needles.getCount();

	// Early stop if all elements in haystack are equal
	if (haystack_min == haystack_max) {
		for (uint64_t i = 0; i < needlesSize; i++) {
			indices[i] = needles[i] == hayStack[0] ? 0 : -1;
		}
		return;
	}

	// Proccess needles in chunks of size 2048
	int chunk_elements_count = (1 << 13) / sizeof(int);
	int* permutation = allocator.alloc<int>(chunk_elements_count);
	const uint64_t chunk_count = needlesSize / chunk_elements_count;
	uint64_t chunk_needles_begin_index = 0;

	for (uint64_t chunk_i = 0; chunk_i <= chunk_count; chunk_i++, chunk_needles_begin_index += chunk_elements_count) {

		// Last chunk has less than 2048 elements
		if (chunk_i == chunk_count) {
			chunk_elements_count = needlesSize - chunk_needles_begin_index;
			// No elements left
			if (chunk_elements_count == 0) {
				break;
			}
		}
		// Fill permutation with {chunk_needles_begin + {1,2,3,4....}}
		for (int j = 0; j < chunk_elements_count; j++) {
			permutation[j] = chunk_needles_begin_index + j;
		}

		// Get permutation of sorted needles
		std::sort(permutation, permutation + chunk_elements_count,
			[&](int a, int b) {
				return needles[a] < needles[b];
			});

		// Bounds on haystack w.r.t. curremt chunk of needles
		uint64_t min_haystack_index = 0;
		uint64_t max_haystack_index = hayStackSize - 1;

		// Needles that are less than haystack_min cannot be found in haystack
		uint64_t needles_begin_index = 0;
		while (needles[permutation[needles_begin_index]] < haystack_min) {
			indices[permutation[needles_begin_index]] = -1;
			needles_begin_index++;
		}

		// Needles that are greater than haystack_max cannot be found in haystack
		uint64_t needles_end_index = chunk_elements_count - 1;
		while (needles[permutation[needles_end_index]] > haystack_max) {
			indices[permutation[needles_end_index]] = -1;
			needles_end_index--;
		}

		uint64_t left_i = needles_begin_index, right_i = needles_end_index;
		// Alternate between searching for minimal or maximal needle
		bool is_min_needle_now = true;
		bool is_first_iter = true;
		// Process needles in chunk
		for (;left_i <= right_i; is_min_needle_now = !is_min_needle_now) {

			int perm_index = is_min_needle_now ? left_i : right_i;

			uint64_t needle_index = permutation[perm_index];
			const int value = needles[needle_index];
			uint64_t left = min_haystack_index;
			uint64_t count = max_haystack_index - min_haystack_index;

			bool found_val = bin_search(
				hayStack,
				needles[needle_index],
				left,
				count);
				
			// After first search for needles[0] - if all needles in chunk are equal
			// Fill them all at once and stop
			if (is_first_iter && needles[permutation[0]] == needles[permutation[chunk_elements_count - 1]]) {
				std::fill_n(indices.get(), needlesSize, found_val ? left:-1);
				is_first_iter = false;
				break;
			}

			// Needles in chunk are in ascending order
			// Keep track of interval in which needles could be found
			if (is_min_needle_now) {
				//min_haystack_index = std::min((uint64_t)0, left - 1);
				min_haystack_index = left;
				left_i++;
			} else {
				//max_haystack_index = std::max((uint64_t)hayStackSize - 1, left + 1);
				max_haystack_index = left;
				right_i--;
			}
			
			if (found_val) {
				indices[needle_index] = left;
			}
			else {
				indices[needle_index] = -1;
			}
		}
